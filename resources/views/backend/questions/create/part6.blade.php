            ...
            <div class="box">
              ...
              <div class="box-body pad">
                ...
                @if ($errors->has('group.*.content'))
                  <span class="text-center help-block">{{$errors->first('group.'.($i-($i-1)).'.content')}}</span>
                @endif
              </div>
              <div class="row">
               @for($j =$i; $j<= ($i+\App\Models\Question::NUMBER_GROUP_PART_6); $j++)
                 <div class="col-md-6">
                   <h4 style ="padding-top: 30px" >{{trans('question.question')}}{{$j}} :</h4>
                   @for($k=0; $k < \App\Models\Answer::NUMBER_ANSWER_4; $k++)
                    <label>
                      <input  type="radio" name="group[{{($i-($i-1))}}][question][{{$j}}][correct]" value="{{$k}}" {{ old('group.'.$i.'.question.'.$j.'.correct').'' == $k.''?'checked':''}}>{{ trans('question.answer'.$k) }}
                      <input type="text" class="radio-inline" name="group[{{($i-($i-1))}}][question][{{$j}}][answer][{{$k}}]"  value="{{old('group.'.($i-($i-1)).'.question.'.$j.'.answer.'.$k)}}" style ="margin-right: 30px; margin-left: 15px;" placeholder="Enter Answer {{trans('question.answer'.$k) }}">
                      @if ($errors->has('group.*.question.*.answer.*'))
                        <span class=" col-md-6 help-block">{{$errors->first('group.'.($i-($i-1)).'.question.'.$j.'.answer.'.$k)}}</span>
                      @endif
                    </label>
                   @endfor
                   @if ($errors->has('group.*.question.*.correct'))
                     <span class="col-md-offset-2 help-block">{{$errors->first('group.'.($i-($i-1)).'.question.'.$j.'.correct')}}</span>
                   @endif
                 </div>
               @endfor
              </div>
